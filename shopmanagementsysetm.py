...
# -------- Simple persistence (share Admin edits to Customers) --------
# Stores data in a local JSON file so different sessions can see updates.
...
def _save_data():
    try:
        ...
        # Don't crash the app; show error only to admin
...
if "qr_prices" not in st.session_state:
    st.session_state.qr_prices = loaded_qr if loaded_qr is not None else {}  # {id: price}
...
if st.session_state.is_admin and not is_product_page:
    ...
    st.session_state.search_query = st.sidebar.text_input(
        "ပစ္စည်းအမည် ရှာဖွေပါ", value=st.session_state.search_query
    )
...
if "product_id" in query_params:
    ...
    product = next(
        (item for item in st.session_state.store_items if item["id"] == product_id), None
    )
        ...
        with col2:
            ...
            if is_admin_access:
                ...
                with st.expander("👨‍💼 Admin (Edit)"):
                    key_inline = st.text_input(
                        "Admin Key", type="password", key=f"inline_admin_key_{product_id}"
                    )
                    if st.button(
                        "Login as Admin",
                        key=f"inline_admin_login_{product_id}",
                        type="primary",
                    ):
        ...
        if is_admin_access:
            ...
            with st.form(f"admin_edit_{product_id}", clear_on_submit=False):
                new_name = st.text_input(
                    "ပစ္စည်းအမည်", value=product["name"], key=f"admin_name_{product_id}"
                )
                new_price = st.text_input(
                    "ဈေးနှုန်း", value=product["price"], key=f"admin_price_{product_id}"
                )
                new_qr_price = st.text_input(
                    "QR Code ဈေးနှုန်း", value=display_price, key=f"admin_qr_price_{product_id}"
                )
                new_img = st.text_input(
                    "ပုံ Link", value=product["img"], key=f"admin_img_{product_id}"
                )

                col_save, col_cancel = st.columns(2)
                with col_save:
                    save_btn = st.form_submit_button(
                        "💾 သိမ်းဆည်းမည်", use_container_width=True, type="primary"
                    )
                with col_cancel:
                    cancel_btn = st.form_submit_button(
                        "❌ ပယ်ဖျက်မည်", use_container_width=True
                    )
                    ...
                    if st.session_state.qr_generating_id == item_id:
                        ...
                        with st.form(f"qr_form_{item_id}", clear_on_submit=False):
                            qr_name = st.text_input(
                                "ပစ္စည်းအမည်", value=item["name"], key=f"qr_name_{item_id}"
                            )
                            qr_price = st.text_input(
                                "ဈေးနှုန်း (ပြင်ဆင်နိုင်သည်)",
                                value=item["price"],
                                key=f"qr_price_{item_id}",
                            )
                            ...
                            with col_cancel_qr:
                                cancel_qr_btn = st.form_submit_button(
                                    "❌ ပယ်ဖျက်မည်", use_container_width=True
                                )
                            ...
                            if generate_btn:
                                if qr_name.strip() and validate_price(qr_price):
                                    ...
                                    st.info(
                                        f"📱 QR Code ကို scan လုပ်လျှင် product page သို့ ရောက်ရှိမည်။\n"
                                        f"💡 QR Code ဈေးနှုန်း: {qr_price} KS\n"
                                        f"📝 မှတ်ချက်: ဈေးနှုန်းပြောင်းလျှင် QR Code အသစ် ထုတ်လုပ်ရန် လိုအပ်ပါသည်။"
                                    )
                                    st.image(
                                        qr_img,
                                        caption=f"{qr_name} - {qr_price} KS",
                                        use_container_width=True,
                                    )
                                    ...
                                    with st.expander("🔗 Product Page URL (Reference)"):
                                        ...
                                        st.warning(
                                            "⚠️ Production တွင် deploy လုပ်လျှင် full domain URL ကို သုံးရန် လိုအပ်ပါသည်။"
                                        )
                        ...
                        with st.form(f"edit_form_{item_id}", clear_on_submit=False):
                            new_name = st.text_input(
                                "ပစ္စည်းအမည်", value=item["name"], key=f"edit_name_{item_id}"
                            )
                            new_price = st.text_input(
                                "ဈေးနှုန်း", value=item["price"], key=f"edit_price_{item_id}"
                            )
                            new_img = st.text_input(
                                "ပုံ Link", value=item["img"], key=f"edit_img_{item_id}"
                            )

                            col_save, col_cancel = st.columns(2)
                            with col_save:
                                save_btn = st.form_submit_button(
                                    "💾 သိမ်းဆည်းမည်", use_container_width=True
                                )
                            with col_cancel:
                                cancel_btn = st.form_submit_button(
                                    "❌ ပယ်ဖျက်မည်", use_container_width=True
...
if "delete_confirm_id" in st.session_state:
    ...
    item_to_delete = next(
        (item for item in st.session_state.store_items if item["id"] == item_id_to_delete),
        None,
    )
