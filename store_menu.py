...
if 'product_id' in query_params:
    ...
    if product:
        ...
        # ပုံနဲ့ ဈေးနှုန်း ဘယ်ညာ တစ်ခြမ်းစီ (ဖုန်းမှာပါ) – admin/customer နှစ်ခုလုံး
        st.markdown(
            """
            <style>
              ...
              .product-page-flex .product-info h2 { margin: 0 0 8px 0 !important; font-size: 1.4em !important; }
              .product-page-flex .product-info .price { margin: 0 !important; font-size: 1.2em !important; font-weight: bold !important; }
              .product-page-flex .product-info .caption { margin-top: 6px !important; font-size: 0.9em !important; opacity: 0.85 !important; }
              ...
              @media screen and (max-width: 480px) {
                .product-page-flex { gap: 10px !important; }
                .product-page-flex .product-info h2 { font-size: 1.2em !important; }
                .product-page-flex .product-info .price { font-size: 1.05em !important; }
                .product-page-flex .product-img-wrap { max-width: 120px !important; }
              }
            </style>
            """,
        ...
        # Single HTML block: ပုံ + ဈေးနှုန်း တစ်ခြမ်းစီ (ဖုန်းမှာပါ ဘယ်ညာ)
        name_esc = html.escape(product['name'])
        price_esc = html.escape(str(display_price))
        caption_html = ""
        if has_qr_price:
            orig_esc = html.escape(str(product['price']))
            caption_html = f'<p class="caption">💡 QR Code ဈေးနှုန်း (မူလဈေးနှုန်း: {orig_esc})</p>'
        img_html = ""
        if cat_img_url:
