...
# Main Interface အတွက် Sidebar ပြမယ်
...
if 'product_id' in query_params:
    ...
    if product:
        ...
        # QR code ဖတ်လို့ရောက်တဲ့အခါ Customer အတွက် အပြည့်အစုံပြမယ်
        # Sidebar ကို ပြန်ဖွင့်ပေးမယ်
        st.sidebar.empty()
        
        # Simple sidebar for product page
        with st.sidebar:
            ...
            if has_qr_price:
                st.caption(f"QR ဈေးနှုန်း (မူလ: {product['price']})")
            
            # Back to menu button
            if st.button("← မီနူးပြန်သွားမည်", use_container_width=True, type="primary"):
            ...
            if not is_admin_access:
                ...
                with st.expander("👨‍💼 Admin (Edit)"):
                    key_inline = st.text_input("Admin Key", type="password", key=f"sidebar_admin_key_{product_id}")
                    if st.button("Login as Admin", key=f"sidebar_admin_login_{product_id}", use_container_width=True):
        ...
        # Product info and image side-by-side (improved layout)
        ...
        with col_info:
            ...
            st.markdown(f"**အမျိုးအစား:** {category_name}")
            
            if is_admin_access:
                st.success("🔐 Admin Mode - ပြင်ဆင်နိုင်ပါသည်။")
            else:
        ...
        with col_img:
            ...
            if cat_img_url:
                try:
                    st.image(cat_img_url, use_container_width=True, caption=f"{category_name}")
                except:
                    st.warning("Image လက်ရှိမရနိုင်ပါ")
            else:
                st.info("Image မရှိပါ")
        
        st.divider()
        
        # Admin Edit Section (in main area)
        if is_admin_access:
...
if not st.session_state.store_items:
    ...
    # Custom CSS for Tea House Menu Style (simplified)
    st.markdown("""
    <style>
    ...
    /* Responsive columns for category layout */
    ...
    for category in categories:
        ...
        if items_in_category:
            ...
            # Category layout - always image on right for all categories
            ...
            with col_items:
                ...
                for item in items_in_category:
                    ...
                    if st.session_state.qr_generating_id == item_id:
                                    ...
                                    if qr_name.strip() and validate_price(qr_price):
                                        ...
                                        if st.session_state.qr_base_url:
                                            ...
                                            try:
                                                ...
                                                if ctx and hasattr(ctx, 'current_page_url'):
                                                    base_url = ctx.current_page_url.split('?')[0]
                                                    full_url = f"{base_url}{product_url}"
                                                else:
                        ...
                        # Display Mode (Menu Format) - Remove ₹ symbol
                        st.markdown(
            ...
            with col_img:
                ...
                if cat_img_url:
                    try:
                        st.image(cat_img_url, use_container_width=True, caption=category_name)
                    except:
                        st.info(f"{category_name} image")
                else:
                    st.info(f"{category_name} image မရှိပါ")
